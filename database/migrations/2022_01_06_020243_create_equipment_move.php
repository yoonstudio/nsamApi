...
class CreateEquipmentMove extends Migration
{
    ...
    public function up()
    {
        Schema::create('equipment_move', function (Blueprint $table) {
            $table->id();
            $table->foreign('ei_seq', 11)->references('ei_seq')->on('equipment_info');
            $table->foreign('ns_tagnumber', 20)->references('ns_tagnumber')->on('equipment_info')->nullable();
            $table->foreign('ns_serialnumber', 50)->references('ns_serialnumber')->on('equipment_info')->nullable();
